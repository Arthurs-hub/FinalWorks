...
class UserRepository extends Repository implements IUserRepository
{
    ...
    public function __construct(Db $db, IDirectoryRepository $directoryRepository)
    {
    ...
    public function getUsersWithPagination(int $offset, int $limit): array
    {
        try {
            ...
            $countStmt = $conn->prepare("SELECT COUNT(*) as count FROM users");
            $countStmt->execute();
    ...
    public function createUserWithRootDirectory(array $userData): int
    {
        ...
        try {
            ...
            $this->directoryRepository->createRootDirectory($userId);
            $conn->commit();
            return $userId;
        } catch (Exception $e) {
            $conn->rollBack();
