...
class FileRepository extends Repository implements IFileRepository
{
    ...
    ";
        ...
        $params = [$userId, $userId, $directoryId, $userId, $userId];
    ...
    public function getFileInfo(int $fileId, int $userId): ?array
    {
        return $this->fetchOne("
            SELECT
                f.*,
                CASE WHEN f.user_id != ? AND si.id IS NOT NULL THEN 1 ELSE 0 END as is_shared
            FROM files f
            LEFT JOIN shared_items si ON si.item_id = f.id AND si.item_type = 'file' AND si.shared_with_user_id = ?
        ...
        ", [$userId, $userId, $fileId, $userId, $userId]);
    }
    ...
    public function getFileInformation(?int $fileId, int $userId): array
    {
        ...
        try {
            $fileInfo = $this->getFileInfo($fileId, $userId);
            if ($fileInfo) {
                if (isset($fileInfo['stored_name'])) {
                    $filePath = __DIR__ . '/../uploads/files/' . $fileInfo['stored_name'];
                    if (file_exists($filePath)) {
                        $fileInfo['file_size_formatted'] = \App\Utils\FileUtils::formatFileSize(filesize($filePath));
                        $fileInfo['preview_available'] = (new \App\Services\FileTypeService())->isPreviewAvailable($fileInfo['mime_type'] ?? '');
                    } else {
            ...
            Logger::error("FileRepository::getFileInformation error", [
                'error' => $e->getMessage(),
                'file_id' => $fileId,
                'user_id' => $userId,
            ]);
    ...
    public function getFileWithAccess(int $fileId, int $userId): array
    {
        try {
            ...
            $file = $this->getFileForDownload($fileId);
            ...
            $hasSharedAccess = $this->checkFileAccess($fileId, $userId);
            ...
            Logger::error("FileRepository::getFileWithAccess error", [
                'error' => $e->getMessage(),
                'file_id' => $fileId,
                'user_id' => $userId,
            ]);
    ...
    /**
     * Returns the full file system path or URL for a stored file name.
     * Adjust this method to return the correct accessible path or URL for the file.
     *
    ...
    {
        $filePath = __DIR__ . '/../uploads/files/' . $storedName;
        return $filePath;
    }
}
