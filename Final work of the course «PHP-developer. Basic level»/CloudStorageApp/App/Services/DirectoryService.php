...
class DirectoryService implements IDirectoryService
{
    ...
    public function __construct(IDirectoryRepository $directoryRepository, IUserRepository $userRepository, DirectoryValidator $directoryValidator)
    {
    ...
    public function deleteDirectory(?string $directoryId, int $userId, bool $isAdmin = false): array
    {
        try {
            ...
            if ($id === null) {
                return ['success' => false, 'error' => 'Нельзя удалить корневую папку'];
            }

            $directory = $this->directoryRepository->getDirectoryByIdPublic($id, $userId, $isAdmin);
            if (!$directory) {
                return ['success' => false, 'error' => 'Папка не найдена'];
            }
