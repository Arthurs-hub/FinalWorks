...
class DirectoryRepository extends Repository
{
    ...
    public function shareDirectory(int $folderId, int $ownerId, string $targetEmail, UserRepository $userService): array
    {
        $conn = $this->db->getConnection();
        ...
        try {
            ...
            $success = $stmt->execute([$folderId, $ownerId, $targetUser['id']]);

            if (!$success) {
                throw new RuntimeException('Ошибка при создании записи о расшаривании');
            }

            $this->shareDirectoryContentsRecursively($conn, $folderId, $ownerId, $targetUser['id']);
